...
def load_mnist(x_train_path, y_train_path, x_valid_path, y_valid_path, train_kind="train", valid_kind="t10k", num_classes=10):
    x_train, y_train = load_single_mnist(x_train_path, y_train_path, kind=train_kind, num_classes=num_classes)
    x_valid, y_valid = load_single_mnist(x_valid_path, y_valid_path, kind=valid_kind, num_classes=num_classes)
    ...
    if y_train is not None: y_train = one_hot(y_train, num_classes=num_classes)
    
    num, params = x_valid.shape[:2]
    if x_valid is not None: x_valid = np.array(x_valid.reshape((num, size, size, 1)), dtype=np.float32)
    if y_valid is not None: y_valid = one_hot(y_valid, num_classes=num_classes)
